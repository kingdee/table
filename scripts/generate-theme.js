...
// We need compile additional content for kd-ui user
function finalizeCompile() {
  if (fs.existsSync(path.join(__dirname, '../lib'))) {
    // Build a entry less file to dist/kd-ui.less
...
function buildThemeFile(theme, vars) {
  // Build less entry file: dist/kd-ui.${theme}.less
  if (theme !== 'default') {
    fs.writeFileSync(
      path.join(process.cwd(), 'dist', `kd-ui.${theme}.less`),
      `@import "../lib/style/${theme}.less";\n@import "../lib/style/components.less";`,
    )
    // eslint-disable-next-line no-console
    console.log(`Built a entry less file to dist/kd-ui.${theme}.less`)
  } else {
    fs.writeFileSync(
...
function writeCompleteLessFile(lessFilesPath) {
  ...
  fs.writeFileSync(path.join(process.cwd(), 'dist', 'kd-ui-complete.less'), allLessContent)
}
...
function finalizeDist() {
  ...
  if (fs.existsSync(path.join(__dirname, '../dist'))) {
    // Build less entry file: dist/kd-ui.less
    fs.writeFileSync(
      path.join(process.cwd(), 'dist', 'kd-ui.less'),
    ...
    // eslint-disable-next-line no-console
    console.log(chalk.blue('Built a entry less file to dist/kd-ui.less'))
